    ...
    <ErrorBoundary>
        <div class="w-full max-w-7xl mx-auto space-y-6 p-4">
            ...
            >
                ...
                <div
                    v-else
                    class="music-table-container rounded-lg shadow-lg overflow-hidden"
                    key="table"
                >
                    <UTable
                    ...
                    <div v-else class="virtual-table-wrapper">
                        <div
                            class="sticky top-0 z-10 bg-blue-900/95 backdrop-blur"
                        >
                            <table class="w-full">
                                <thead>
                                    <tr>
                        ...
                        >
                            <tr
                                ...
                                :class="{
                                    'bg-blue-900/20':
                                        virtualItem.index === selectedRowIndex,
                                }"
                                :tabindex="
                                    virtualItem.index === selectedRowIndex
                                        ? 0
...
const { smoothScrollToRow, handleTouchStart, handleTouchMove } =
    useTableAnimations();
...
const onRowSelect = (piece: Piece) => {
    try {
        ...
        selectedRowIndex.value = paginatedItems.value.findIndex(
            (p) => p.stid === piece.stid,
        );
    } catch (err) {
        setError({
    ...
    nextTick(() => {
        if (shouldUseVirtualScroll.value && virtualTable.value) {
            ...
            const rows = document.querySelectorAll(
                ".music-table-container tbody tr",
            );
...
const tableUI = {
    ...
    loadingWrapper:
        "flex flex-col items-center justify-center flex-1 px-6 py-14",
