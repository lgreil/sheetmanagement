<template>
  <div class="music-table-container rounded-lg overflow-hidden" v-auto-animate>
    <div class="overflow-x-auto">
      <table class="w-full">
        <thead>
          <tr class="border-b border-blue-900/30">
            <th v-for="(width, i) in columnWidths" :key="i" class="p-3">
              <USkeleton class="h-4" :class="width" />
            </th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="n in 10" :key="n" class="border-b border-blue-900/30">
            <td v-for="(content, i) in rowContents" :key="i" class="p-3">
              <template v-if="content.type === 'text'">
                <USkeleton class="h-4" :class="content.width" />
              </template>
              <template v-else-if="content.type === 'badge'">
                <USkeleton class="h-6 rounded-full" :class="content.width" />
              </template>
              ...
              <template v-else-if="content.type === 'actions'">
                <div class="flex gap-2">
                  <USkeleton v-for="j in 2" :key="j" class="h-8 w-8 rounded-lg" />
                </div>
              </template>
            </td>
          </tr>
    ...
    <!-- Pagination skeleton -->
    <div class="p-4 flex items-center justify-between border-t border-blue-900/30">
      <USkeleton class="h-5 w-32" />
      <div class="flex items-center gap-4">
        <div class="flex gap-1">
          <USkeleton v-for="n in 5" :key="n" class="h-9 w-9 rounded-lg" />
        </div>
